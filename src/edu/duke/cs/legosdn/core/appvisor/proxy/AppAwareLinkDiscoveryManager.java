...
public class AppAwareLinkDiscoveryManager
        implements IOFMessageListener,
                   IOFSwitchListener,
                   IStorageSourceListener,
                   IAppAwareLinkDiscoveryService,
                   IFloodlightModule,
                   IInfoProvider {
    protected static final Logger               log      =
            LoggerFactory.getLogger(AppAwareLinkDiscoveryManager.class);
    protected static final INotificationManager notifier =
            NotificationManagerFactory.getNotificationManager(
                    net.floodlightcontroller.linkdiscovery.internal.LinkDiscoveryManager.class);
    ...
    /**
     * Boolean indicating if the link discovery manager is app-aware or not.
     */
    protected volatile boolean appAware = true;

    /**
     * Per-application network state information.
     */
    protected Map<String, Set<Long>>                     paSwitchesDel;
    protected Map<String, Map<Link, LinkInfo>>           paLinksDel;
    protected Map<String, Map<Long, Set<Link>>>          paSwitchLinksDel;
    protected Map<String, Map<NodePortTuple, Set<Link>>> paPortLinksDel;
    ...
    private final String PACKAGE =
            net.floodlightcontroller.linkdiscovery.internal.LinkDiscoveryManager.class.getPackage().getName();
    ...
    public Map<Link, LinkInfo> getLinks(String appId) {
        ...
        Map<Link, LinkInfo> paResult = new HashMap<Link, LinkInfo>();
        Map<Link, LinkInfo> paSkipResult = this.paLinksDel.get(appId);
        for (Link link : result.keySet()) {
            if (!paSkipResult.containsKey(link)) {
                paResult.put(link, result.get(link));
            } else {
                if (log.isInfoEnabled()) {
                    log.info(String.format("getLinks> skipping link Sw-%d:%d => Sw-%d:%d",
                                           link.getSrc(), link.getSrcPort(), link.getDst(), link.getDstPort()));
                }
            }
        }
    ...
    public void addLink(Link link, LinkInfo linkInfo, String appId) {
        ...
        this.paLinksDel.get(appId).remove(link);
    }
    ...
    public void delLink(Link link, LinkInfo linkInfo, String appId) {
        ...
        this.paLinksDel.get(appId).put(link, linkInfo);
    }

    @Override
    public Map<Link, LinkInfo> getDelLinks(String appId) {
        Map<Link, LinkInfo> delLinks = new HashMap<Link, LinkInfo>();
        if (!this.appAware)
            return delLinks;

        delLinks.putAll(this.paLinksDel.get(appId));
        return delLinks;
    }
    ...
    public Set<Long> getSwitchesDel(String appId) {
        ...
        final Set<Long> sws = this.paSwitchesDel.get(appId);
        final int sz =  sws == null ? 0 : sws.size();
        final Set<Long> delSwitches = new HashSet<Long>(sz);
        if (sz > 0)
            delSwitches.addAll(sws);
        return delSwitches;
    }
    ...
    public void setSwitchUp(Long swID, String appId) {
        ...
        if (!this.paSwitchesDel.containsKey(appId) ||
            !this.paSwitchesDel.get(appId).contains(swID))
            return;
        this.paSwitchesDel.get(appId).remove(swID);

        if (log.isDebugEnabled()) {
            log.debug(String.format("setSwitchDown> Sw-%d state set to UP for %s", swID, appId));
        }
    }
    ...
    public void setSwitchDown(Long swID, String appId) {
        ...
        if (!this.paSwitchesDel.containsKey(appId))
            this.paSwitchesDel.put(appId, new HashSet<Long>());
        this.paSwitchesDel.get(appId).add(swID);

        if (log.isDebugEnabled()) {
            log.debug(String.format("setSwitchDown> Sw-%d state set to DOWN for %s", swID, appId));
        }
    }
    ...
    public void registerEndpt(String appId) {
        ...
        if (this.paLinksDel.containsKey(appId))
            return;

        this.paLinksDel.put(appId, new HashMap<Link, LinkInfo>());
        this.paSwitchLinksDel.put(appId, new HashMap<Long, Set<Link>>());
        this.paPortLinksDel.put(appId, new HashMap<NodePortTuple, Set<Link>>());

        if (log.isInfoEnabled()) {
            log.info(String.format("registerEndpt> registered '%s'.", appId));
        }
    }
...
//                                           cntx);
                ...
                // NOTE: Ignore the return value from handlePacketIn
                this.handlePacketIn(sw.getId(), (OFPacketIn) msg,
                                    cntx);
            default:
                break;
        }
        ...
        try {
            ...
            if (oldInfo == null) {
                ...
                if (linkType == ILinkDiscovery.LinkType.DIRECT_LINK) {
                    log.info("Inter-switch link detected: {}; #links {}", lt, links.size());
    ...
    public void init(FloodlightModuleContext context)
        ...
        this.paSwitchesDel = new ConcurrentHashMap<String, Set<Long>>();
        this.paLinksDel = new ConcurrentHashMap<String, Map<Link, LinkInfo>>();
        this.paSwitchLinksDel = new ConcurrentHashMap<String, Map<Long, Set<Link>>>();
        this.paPortLinksDel = new ConcurrentHashMap<String, Map<NodePortTuple, Set<Link>>>();
    ...
    private class HAListenerDelegate implements IHAListener {
        ...
        public void transitionToMaster() {
            ...
            AppAwareLinkDiscoveryManager.this.role = Role.MASTER;
            clearAllLinks();
            readTopologyConfigFromStorage();
        ...
        @Override
        public String getName() {
            return AppAwareLinkDiscoveryManager.this.getName();
        }
